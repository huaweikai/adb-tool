...
func (s *AdbEventStream) Run(ctx context.Context) {
	...
	s.logger.Printf("starting, port=%d", s.port)
...
func (s *AdbEventStream) connectAndStream(ctx context.Context) error {
	...
	s.logger.Printf("connecting to %s", addr)
	...
	s.logger.Printf("connected, reading events...")
...
func (s *AdbEventStream) handlePayload(payload []byte) error {
	...
	// Detect state changes (e.g. "authorizing" → "device") even when
	// the serial is unchanged.  Without this, a device that was already
	// in the list but changes state would be silently ignored.
	stateChanged := false
	if prev != nil {
		for serial, curr := range current {
			if old, ok := prev[serial]; ok && curr.State != old.State {
				stateChanged = true
				break
			}
		}
	...
	if len(added) > 0 || len(removed) > 0 || stateChanged {
		sort.Strings(added)
