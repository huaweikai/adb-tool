...
![Android 模拟器](assets/android_emulator_screen.png)

### 测试配置

![测试配置](assets/test_config_screen.png)

### 测试会话

![测试会话](assets/test_session.png)

### 设置

![设置](assets/settings_screen.png)

## 主要功能

- **设备管理**：自动发现 USB / WiFi 连接的 Android 设备，展示设备型号、品牌、系统版本和连接状态。
- **设备投屏**：通过内置 scrcpy 镜像设备屏幕，支持同时投屏多台设备；视频由独立 scrcpy 窗口或后台无窗口录制显示；支持按设备保存视频、音频、窗口、控制、设备和录制参数，并提供快捷键参考。
...
- **设置**：提供统一的设置页面，支持亮色/暗色主题切换、中文/英文界面切换、录屏存储路径与录制方式配置等。
- **命令面板**：支持 Cmd/Ctrl+K 快速打开命令面板，提供键盘快捷键导航和全局操作入口，提升操作效率。
